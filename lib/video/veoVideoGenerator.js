 ...
 */
function extractVideoBuffer(generatedVideo) {
  if (!generatedVideo) return null;

  // 1. Direct Buffer / Uint8Array
  ...
  // 3. Inspect properties on generatedVideo and generatedVideo.video
  // Note: @google/genai SDK maps Vertex AI's bytesBase64Encoded -> video.videoBytes
  const candidates = [
 ...
 * Retries transient download/network failures up to MAX_DOWNLOAD_ATTEMPTS with exponential backoff.
 *
 ...
 */
async function saveVideoFile(generatedVideo, outputPath) {
  const targetDir = path.dirname(outputPath);
  if (!fs.existsSync(targetDir)) {
    fs.mkdirSync(targetDir, { recursive: true });
  }

  // 1. Try direct video buffer extraction (e.g. video.videoBytes from @google/genai SDK)
  ...
  // 2. Try URI download (HTTP / HTTPS / GCS URI) with dedicated retry loop and generous timeouts
  const videoUri =
      ...
      try {
        ...
        if (videoUri.startsWith('gs://')) {
          ...
          if (gcsMatch) {
            ...
            let gcsResponse;
            try {
              gcsResponse = await fetch(publicHttpsUrl, { signal: controller.signal });
            } finally {
              clearTimeout(timeoutId);
            }
  ...
  // 3. Try using the GenAI SDK file downloader if available
  if (ai.files && typeof ai.files.download === 'function') {
    try {
      const fileToDownload = generatedVideo?.video || generatedVideo;
      if (fileToDownload && (fileToDownload.name || typeof fileToDownload === 'string')) {
        ...
        if (fs.existsSync(outputPath) && fs.statSync(outputPath).size > 0) {
          console.log(`[veoVideoGenerator] Downloaded file via GenAI SDK files.download.`);
          return;
        }
      }
  ...
  if (!fs.existsSync(outputPath) || fs.statSync(outputPath).size === 0) {
    const diagnosticSummary = {
      ...
      hasVideoBytes: Boolean(generatedVideo?.video?.videoBytes || generatedVideo?.videoBytes),
      hasUri: Boolean(generatedVideo?.video?.uri || generatedVideo?.uri)
    };
...
export async function generateVideoClip({ prompt, outputPath, shotNumber = 1, options = {}, onProgress }) {
  ...
  for (let attempt = 1; attempt <= MAX_TRANSIENT_ATTEMPTS; attempt++) {
    ...
    console.log('\n====================================================');
    console.log(`[veoVideoGenerator] Starting Veo Request for Shot #${shotNumber} (Attempt ${attempt}/${MAX_TRANSIENT_ATTEMPTS})`);
...
export { extractVideoBuffer, saveVideoFile };
