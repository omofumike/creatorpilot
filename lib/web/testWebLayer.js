...
async function runWebLayerTests() {
  ...
  try {
    ...
    assertCondition('GET / contains "Judge\'s Dashboard"', getHomeBody.includes("Judge's Dashboard"));
    console.log('');
    ...
    // --------------------------------------------------
    // TEST 5: API credentials are never exposed in error responses
    // --------------------------------------------------
    console.log('--- TEST 5: Credential redacting in errors ---');
