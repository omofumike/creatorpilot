...
export async function reviewQuality(researchBrief, script, storyboard, production, options = {}) {
  ...
  if (!actualScript.title || !Array.isArray(actualScript.sections)) {
...
JSON Schema Required Keys:
{
  ...
  "productionIssues": [
    {
      "item": "Specific filming step, filming checklist task, equipment, location, or warning.",
      "severity": "WARNING | FAIL",
      "finding": "Explanation of why it violates solo-creator feasibility (e.g., assumes crew, assistant, camera operator, multiple presenters, actors, professional multi-camera setups, expensive studios, complex setups).",
      "recommendedAction": "Pragmatic advice to make it easily capturing-ready for a single person working alone with consumer-grade equipment."
    }
  ],
...
5. SOLO CREATOR FEASIBILITY:
- Verify that every visual instruction, shot setup, and equipment requirement is highly practical and executable by a SINGLE person working alone.
- If you find any references to camera operators, crew members, assistants, actors, professional studio sets, or high-end specialized equipment, flag this as a WARNING or FAIL in the production category.
...
- Section #${idx + 1}: ${sec.heading}
  ...
  Source References: ${(sec.sourceReferences || []).join('; ')}
`).join('\n')}
...
- Shot #${s.shotNumber} [${s.shotType}]
  ...
  Est Duration: ${s.estimatedDuration || 'N/A'}
  ...
  Production Notes: ${s.productionNotes}
  Source References: ${(s.sourceReferences || []).join(', ')}
`).join('\n')}

=========================================
4. PRODUCTION PLAN (STAGES 5 & 6)
=========================================
Title: ${actualProduction.title}
Platform: ${actualProduction.platform || 'N/A'}
...
- Shot #${s.shotNumber} (Storyboard Shot #${s.storyboardShotNumber}) [${s.shotType}]
  ...
  Setup Instructions: ${s.setupInstructions}
`).join('\n')}
