...
export async function researchTopic(topic, options = {}) {
  ...
  if (!searchResult || !searchResult.success) {
    const errorMsg = searchResult?.error || 'Parallel Search failed: Connection error.';
    return {
      success: false,
      failedStage: 'research',
      error: errorMsg,
      step: 'research-search-failure',
      topic: queryTopic,
      diagnostics: searchResult?.diagnostics || null
    };
  }

  // 3. Format the search results text block for Gemini
...
- Do not attribute commonly-assumed properties of a product category to specific claims unless the supplied search evidence explicitly states that property for that claim.
...
Please synthesize the search results above and generate the comprehensive research brief strictly adhering to the JSON schema.`;
  ...
  // 4. Query Gemini with structured output configurations
  let responseText = '';
  try {
    ...
    // 5. Return successful structured response
    return {
      success: true,
      topic: queryTopic,
      searchMetadata: {
    ...
    const redactedError = redactApiKeys(geminiError.message || String(geminiError));
