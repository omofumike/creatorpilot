...
export function redactApiKeys(message) {
  ...
  const keysToRedact = [config.geminiApiKey, config.googleApiKey, config.parallelApiKey].filter(Boolean);
  for (const key of keysToRedact) {
    // Only redact if the key is a non-empty string
    if (typeof key === 'string' && key.length > 0) {
      const escapedKey = key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      redacted = redacted.replace(new RegExp(escapedKey, 'g'), '[REDACTED_API_KEY]');
    }
  }
  return redacted;
}
